...
def add_request(name, branch, year, roll, email, parent_number, reason, out_time, date):
    conn = get_conn()
    c = conn.cursor()
    ...
    c.execute(
        ...
        (name, branch, year, roll, email, parent_number, reason, out_time, date,
...
def start_hod_timeout_watchdog(req_id, class_email, emergency_url):
    ...
    def worker():
        ...
        # New index map:
        # 0:id, 1:name, 2:branch, 3:year, 4:roll, 5:email, 6:parent_number,
        # 7:reason, 8:out_time, 9:date, 10:class_status, 11:hod_status,
        # 12:principal_status, 13:parent_confirm, 14:created_at,
        # 15:class_note, 16:used_status, 17:sent_to_hod_at
...
<div style="margin-top:10px;">
  <button type="button" onclick="sendOTP()" 
    ...
    Send OTP
                ...
                <input id="parent_number" name="parent_number" required placeholder="Parent Mobile Number" maxlength="10">
...
def submit():
    ...
    parent_number = request.form["parent_number"].strip()
    ...
    req_id = add_request(name, branch, year, roll, email, parent_number, reason, out_time, date)
    ...
    <b>Roll:</b> {roll}<br>
    <b>Email:</b> {email}<br>
    <b>Parent No.:</b> {parent_number}<br>
    ...
    # Student ko confirmation (parent number yaha nahi jayega)
    send_email(
        email,
        "Gate Pass Submitted",
...
def class_approve(req_id):
    ...
    # index mapping:
    # 0:id 1:name 2:branch 3:year 4:roll 5:email 6:parent_number 7:reason 8:out_time 9:date
    # 10:class_status 11:hod_status 12:principal_status
    # 13:parent_confirm 14:created_at 15:class_note 16:used_status 17:sent_to_hod_at
    ...
    if request.method == "POST":
        ...
        if action == "approve":
            ...
            <div style='border:2px solid #0084ff; padding:10px; border-radius:8px; background:#e7f1ff;'>
              <b>📌 Note from Class Incharge:</b><br>{data[15] or 'No note given.'}
            </div><br>
            """
            ...
            <b>Roll:</b> {data[4]}<br>
            <b>Email:</b> {data[5]}<br>
            <b>Parent No.:</b> {data[6]}<br>
            <b>Reason:</b> {data[7]}<br>
            <b>Out Time:</b> {data[8]}<br>
            <b>Date:</b> {data[9]}<br>
            <b>Class Incharge Status:</b> {data[10]}<br>
            </p>
            ...
            # Student notification (parent number yaha bhi nahi jayega)
            send_email(
                data[5],
    ...
    <form method="POST">
      ...
      <textarea name="note" rows="4" cols="50" placeholder="Parents se call par baat ho gayi...">{data[15] or ""}</textarea>
      <br><br>
...
def hod_approve(req_id):
    ...
    # Student notification (parent number nahi)
    send_email(
        data[5],
...
def final_approve(req_id):
    ...
    # 12 = principal_status (final approval status)
    if data[12] == "Approved":
    ...
    # Student notification with final QR (parent number nahi)
    send_email(
        data[5],
...
def qrcode_route(req_id):
    ...
    if data[11] != "Approved":  # 11 = hod_status
        return """
    ...
    qr_data = (
        ...
        f"Roll:{data[4]} | Class:{data[10]} | HOD:{data[11]} | Final:{data[12]}"
    )
...
def verify_qr():
    ...
    if (
        req[10] == "Approved" and    # Class Incharge
        req[11] == "Approved" and   # HOD
        req[12] == "Approved"       # Final (Principal or Registrar)
    ):
        ...
        <p><b>Reason:</b> {req[7]}</p>
        <p><b>Out Time:</b> {req[8]}</p>
        <p><b>Date:</b> {req[9]}</p>
        <p><b>Class Incharge:</b> {req[10]}</p>
        <p><b>HOD:</b> {req[11]}</p>
        <p><b>Final Approval:</b> {req[12]}</p>
        """
    else:
...
def admin():
    ...
    html += "<tr><th>ID</th><th>Name</th><th>Year</th><th>Roll</th><th>Parent No.</th><th>Class</th><th>HOD</th><th>Final</th><th>Used?</th></tr>"

    for r in rows:
        html += f"<tr><td>{r[0]}</td><td>{r[1]}</td><td>{r[3]}</td><td>{r[4]}</td><td>{r[6]}</td><td>{r[10]}</td><td>{r[11]}</td><td>{r[12]}</td><td>{r[16]}</td></tr>"
